...
import com.valb3r.bpmn.intellij.plugin.bpmn.api.bpmn.elements.ExtensionFromEvent
...
import com.valb3r.bpmn.intellij.plugin.flowable.parser.nodes.collaboration.Collaboration
...
const val EXTENSION_ELEM_STREAM =
    "java(null == input.getExtensionElements() ? null : input.getExtensionElements().stream()"
const val EXTENSION_STRING_EXTRACTOR =
    ".map(it -> it.getString()).filter(java.util.Objects::nonNull).findFirst().orElse(null))"
const val UNMAPPED_ELEM_STREAM =
    "java(null == input.getUnmappedProperties() ? null : input.getUnmappedProperties().stream()"
const val UNMAPPED_STRING_EXTRACTOR =
    ".map(it -> it.getString()).filter(java.util.Objects::nonNull).findFirst().orElse(null))"
const val EXTENSION_EXPRESSION_EXTRACTOR =
    ".map(it -> it.getExpression()).filter(java.util.Objects::nonNull).findFirst().orElse(null))"
...
class ProcessNode : BpmnMappable<BpmnProcess>, ProcessBody() {
    ...
    private fun mapChildren(
        id: BpmnElementId,
        body: ProcessBody,
        children: MutableMap<BpmnElementId, BpmnProcessBody>
    ) {
    ...
    private fun applyServiceTaskCustomizationByType(process: BpmnProcessBody): BpmnProcessBody {
        ...
        result = extractTasksBasedOnType(
            result,
            "camel",
            cachedMapper(CamelMapper::class.java)
        ) { updates, target -> target.copy(camelTask = updates) }
        result = extractTasksBasedOnType(
            result,
            "http",
            cachedMapper(HttpMapper::class.java)
        ) { updates, target -> target.copy(httpTask = updates) }
        result = extractTasksBasedOnType(
            result,
            "external-worker",
            cachedMapper(ExternalTaskMapper::class.java)
        ) { updates, target -> target.copy(externalTask = updates) }
        result = extractTasksBasedOnType(
            result,
            "mail",
            cachedMapper(MailMapper::class.java)
        ) { updates, target -> target.copy(mailTask = updates) }
        result = extractTasksBasedOnType(
            result,
            "mule",
            cachedMapper(MuleMapper::class.java)
        ) { updates, target -> target.copy(muleTask = updates) }
        result = extractTasksBasedOnType(
            result,
            "dmn",
            cachedMapper(DecisionMapper::class.java)
        ) { updates, target -> target.copy(decisionTask = updates) }
        result = extractTasksBasedOnType(
            result,
            "shell",
            cachedMapper(ShellMapper::class.java)
        ) { updates, target -> target.copy(shellTask = updates) }
        result = extractTasksBasedOnType(
            result,
            "send-event",
    ...
    private fun applySubprocessCustomizationByEventTrigger(process: BpmnProcessBody): BpmnProcessBody {
        ...
        process.subProcess?.apply {
            ...
            return process.copy(
                subProcess = byTypeGroup[false],
                eventSubProcess = byTypeGroup[true]?.map { mapper.convertToDto(it) })
        }
        return process
    }
    ...
    private fun applyIntermediateCatchEventCustomizationByType(process: BpmnProcessBody): BpmnProcessBody {
        var result = process
        result = extractIntermediateCatchEventsBasedOnType(
            result,
            { null != it.timerEventDefinition },
            cachedMapper(TimerCatchingMapper::class.java)
        ) { updates, target -> target.copy(intermediateTimerCatchingEvent = updates) }
        result = extractIntermediateCatchEventsBasedOnType(
            result,
            { null != it.signalEventDefinition },
            cachedMapper(SignalCatchingMapper::class.java)
        ) { updates, target -> target.copy(intermediateSignalCatchingEvent = updates) }
        result = extractIntermediateCatchEventsBasedOnType(
            result,
            { null != it.messageEventDefinition },
            cachedMapper(MessageCatchingMapper::class.java)
        ) { updates, target -> target.copy(intermediateMessageCatchingEvent = updates) }
        result = extractIntermediateCatchEventsBasedOnType(
            result,
            { null != it.conditionalEventDefinition },
            cachedMapper(ConditionalCatchingMapper::class.java)
        ) { updates, target -> target.copy(intermediateConditionalCatchingEvent = updates) }
        return result
    }

    private fun applyIntermediateThrowingEventCustomizationByType(process: BpmnProcessBody): BpmnProcessBody {
        var result = process
        result = extractIntermediateThrowingEventsBasedOnType(
            result,
            { null == it.escalationEventDefinition && null == it.signalEventDefinition },
            cachedMapper(NoneThrowMapper::class.java)
        ) { updates, target -> target.copy(intermediateNoneThrowingEvent = updates) }
        result = extractIntermediateThrowingEventsBasedOnType(
            result,
            { null != it.signalEventDefinition },
            cachedMapper(SignalThrowMapper::class.java)
        ) { updates, target -> target.copy(intermediateSignalThrowingEvent = updates) }
        result = extractIntermediateThrowingEventsBasedOnType(
            result,
            { null != it.escalationEventDefinition },
            cachedMapper(EscalationThrowMapper::class.java)
        ) { updates, target -> target.copy(intermediateEscalationThrowingEvent = updates) }
        return result
    }

    private fun applyEndEventCustomizationByType(process: BpmnProcessBody): BpmnProcessBody {
        var result = process
        result = extractEndEventsBasedOnType(
            result,
            { null != it.errorEventDefinition },
            cachedMapper(EndErrorMapper::class.java)
        ) { updates, target -> target.copy(errorEndEvent = updates) }
        result = extractEndEventsBasedOnType(
            result,
            { null != it.escalationEventDefinition },
            cachedMapper(EndEscalationMapper::class.java)
        ) { updates, target -> target.copy(escalationEndEvent = updates) }
        result = extractEndEventsBasedOnType(
            result,
            { null != it.cancelEventDefinition },
            cachedMapper(EndCancelMapper::class.java)
        ) { updates, target -> target.copy(cancelEndEvent = updates) }
        result = extractEndEventsBasedOnType(
            result,
            { null != it.terminateEventDefinition },
            cachedMapper(EndTerminationMapper::class.java)
        ) { updates, target -> target.copy(terminateEndEvent = updates) }
        return result
    }

    private fun applyStartEventCustomizationByType(process: BpmnProcessBody): BpmnProcessBody {
        var result = process
        result = extractStartEventsBasedOnType(
            result,
            { null != it.conditionalEventDefinition },
            cachedMapper(StartConditionalMapper::class.java)
        ) { updates, target -> target.copy(conditionalStartEvent = updates) }
        result = extractStartEventsBasedOnType(
            result,
            { null != it.errorEventDefinition },
            cachedMapper(StartErrorMapper::class.java)
        ) { updates, target -> target.copy(errorStartEvent = updates) }
        result = extractStartEventsBasedOnType(
            result,
            { null != it.escalationEventDefinition },
            cachedMapper(StartEscalationMapper::class.java)
        ) { updates, target -> target.copy(escalationStartEvent = updates) }
        result = extractStartEventsBasedOnType(
            result,
            { null != it.messageEventDefinition },
            cachedMapper(StartMessageMapper::class.java)
        ) { updates, target -> target.copy(messageStartEvent = updates) }
        result = extractStartEventsBasedOnType(
            result,
            { null != it.signalEventDefinition },
            cachedMapper(StartSignalMapper::class.java)
        ) { updates, target -> target.copy(signalStartEvent = updates) }
        result = extractStartEventsBasedOnType(
            result,
            { null != it.timerEventDefinition },
            cachedMapper(StartTimerMapper::class.java)
        ) { updates, target -> target.copy(timerStartEvent = updates) }
        return result
    }

    private fun applyBoundaryEventCustomizationByType(process: BpmnProcessBody): BpmnProcessBody {
        var result = process
        result = extractBoundaryEventsBasedOnType(
            result,
            { null != it.cancelEventDefinition },
            cachedMapper(BoundaryCancelMapper::class.java)
        ) { updates, target -> target.copy(boundaryCancelEvent = updates) }
        result = extractBoundaryEventsBasedOnType(
            result,
            { null != it.compensateEventDefinition },
            cachedMapper(BoundaryCompensationMapper::class.java)
        ) { updates, target -> target.copy(boundaryCompensationEvent = updates) }
        result = extractBoundaryEventsBasedOnType(
            result,
            { null != it.conditionalEventDefinition },
            cachedMapper(BoundaryConditionalMapper::class.java)
        ) { updates, target -> target.copy(boundaryConditionalEvent = updates) }
        result = extractBoundaryEventsBasedOnType(
            result,
            { null != it.errorEventDefinition },
            cachedMapper(BoundaryErrorMapper::class.java)
        ) { updates, target -> target.copy(boundaryErrorEvent = updates) }
        result = extractBoundaryEventsBasedOnType(
            result,
            { null != it.escalationEventDefinition },
            cachedMapper(BoundaryEscalationMapper::class.java)
        ) { updates, target -> target.copy(boundaryEscalationEvent = updates) }
        result = extractBoundaryEventsBasedOnType(
            result,
            { null != it.messageEventDefinition },
            cachedMapper(BoundaryMessageMapper::class.java)
        ) { updates, target -> target.copy(boundaryMessageEvent = updates) }
        result = extractBoundaryEventsBasedOnType(
            result,
            { null != it.signalEventDefinition },
            cachedMapper(BoundarySignalMapper::class.java)
        ) { updates, target -> target.copy(boundarySignalEvent = updates) }
        result = extractBoundaryEventsBasedOnType(
            result,
            { null != it.timerEventDefinition },
            cachedMapper(BoundaryTimerMapper::class.java)
        ) { updates, target -> target.copy(boundaryTimerEvent = updates) }
        return result
    }

    private fun <T> extractTasksBasedOnType(
        process: BpmnProcessBody,
        type: String,
        mapper: ServiceTaskMapper<T>,
        update: (List<T>?, BpmnProcessBody) -> BpmnProcessBody
    ): BpmnProcessBody {
    ...
    private fun <T> extractIntermediateCatchEventsBasedOnType(
        process: BpmnProcessBody,
        filter: (BpmnIntermediateCatchingEvent) -> Boolean,
        mapper: IntermediateCatchEventMapper<T>,
        update: (List<T>?, BpmnProcessBody) -> BpmnProcessBody
    ): BpmnProcessBody {
    ...
    private fun <T> extractIntermediateThrowingEventsBasedOnType(
        process: BpmnProcessBody,
        filter: (BpmnIntermediateThrowingEvent) -> Boolean,
        mapper: IntermediateThrowEventMapper<T>,
        update: (List<T>?, BpmnProcessBody) -> BpmnProcessBody
    ): BpmnProcessBody {
    ...
    private fun <T> extractEndEventsBasedOnType(
        process: BpmnProcessBody,
        filter: (BpmnEndEvent) -> Boolean,
        mapper: EndEventMapper<T>,
        update: (List<T>?, BpmnProcessBody) -> BpmnProcessBody
    ): BpmnProcessBody {
    ...
    private fun <T> extractStartEventsBasedOnType(
        process: BpmnProcessBody,
        filter: (BpmnStartEvent) -> Boolean,
        mapper: StartEventMapper<T>,
        update: (List<T>?, BpmnProcessBody) -> BpmnProcessBody
    ): BpmnProcessBody {
    ...
    private fun <T> extractBoundaryEventsBasedOnType(
        process: BpmnProcessBody,
        filter: (BpmnBoundaryEvent) -> Boolean,
        mapper: BoundaryEventMapper<T>,
        update: (List<T>?, BpmnProcessBody) -> BpmnProcessBody
    ): BpmnProcessBody {
    ...
    interface CamelMapper : ServiceTaskMapper<BpmnCamelTask> {
        ...
        @Mappings(
            ...
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"camelContext\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "camelContext"
            )
        )
    ...
    interface HttpMapper : ServiceTaskMapper<BpmnHttpTask> {
        ...
        @Mappings(
            Mapping(source = "forCompensation", target = "isForCompensation"),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"requestMethod\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "requestMethod"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"requestUrl\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "requestUrl"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"requestHeaders\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "requestHeaders"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"requestBody\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "requestBody"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"requestBodyEncoding\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "requestBodyEncoding"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"requestTimeout\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "requestTimeout"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"disallowRedirects\".equals(it.getName()))$EXTENSION_BOOLEAN_EXTRACTOR",
                target = "disallowRedirects"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"failStatusCodes\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "failStatusCodes"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"handleStatusCodes\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "handleStatusCodes"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"responseVariableName\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "responseVariableName"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"ignoreException\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "ignoreException"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"saveRequestVariables\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "saveRequestVariables"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"saveResponseParameters\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "saveResponseParameters"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"resultVariablePrefix\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "resultVariablePrefix"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"saveResponseParametersTransient\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "saveResponseParametersTransient"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"saveResponseVariableAsJson\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
    ...
    interface ExternalTaskMapper : ServiceTaskMapper<BpmnExternalTask> {
        @Mappings(
            Mapping(source = "forCompensation", target = "isForCompensation"),
            Mapping(
                expression = "$UNMAPPED_ELEM_STREAM.filter(it -> \"jobTopic\".equals(it.getName()))$UNMAPPED_STRING_EXTRACTOR",
                target = "jobTopic"
            ),
        )
    ...
    interface MailMapper : ServiceTaskMapper<BpmnMailTask> {
        ...
        @Mappings(
            Mapping(source = "forCompensation", target = "isForCompensation"),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"headers\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "headers"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"to\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "to"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"from\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "from"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"subject\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "subject"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"cc\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "cc"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"bcc\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "bcc"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"text\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "text"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"html\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "html"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"charset\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "charset"
            )
        )
    ...
    interface MuleMapper : ServiceTaskMapper<BpmnMuleTask> {
        @Mappings(
            Mapping(source = "forCompensation", target = "isForCompensation"),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"endpointUrl\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "endpointUrl"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"language\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "language"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"payloadExpression\".equals(it.getName()))$EXTENSION_EXPRESSION_EXTRACTOR",
                target = "payloadExpression"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"resultVariable\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
    ...
    interface DecisionMapper : ServiceTaskMapper<BpmnDecisionTask> {
        ...
        @Mappings(
            Mapping(source = "forCompensation", target = "isForCompensation"),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"decisionTableReferenceKey\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "decisionTableReferenceKey"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"decisionTaskThrowErrorOnNoHits\".equals(it.getName()))$EXTENSION_BOOLEAN_EXTRACTOR",
                target = "decisionTaskThrowErrorOnNoHits"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"fallbackToDefaultTenant\".equals(it.getName()))$EXTENSION_BOOLEAN_EXTRACTOR",
    ...
    interface ShellMapper : ServiceTaskMapper<BpmnShellTask> {
        ...
        @Mappings(
            Mapping(source = "forCompensation", target = "isForCompensation"),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"command\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "command"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"arg1\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "arg1"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"arg2\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "arg2"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"arg3\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "arg3"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"arg4\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "arg4"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"arg5\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "arg5"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"wait\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "wait"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"cleanEnv\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "cleanEnv"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"errorCodeVariable\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "errorCodeVariable"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"outputVariable\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "outputVariable"
            ),
            Mapping(
                expression = "$EXTENSION_ELEM_STREAM.filter(it -> \"directory\".equals(it.getName()))$EXTENSION_STRING_EXTRACTOR",
                target = "directory"
            )
        )
