...
fun readAndUpdateProcess(parser: FlowableParser, event: EventPropagatableToXml): BpmnFileObject {
    return readAndUpdateProcess(parser, "simple-nested.bpmn20.xml", event)
}

fun readAndUpdateProcess(parser: FlowableParser, processName: String, event: EventPropagatableToXml): BpmnFileObject {
    return readAndUpdateProcess(parser, processName, listOf(event))
}

fun readAndUpdateProcess(parser: FlowableParser, processName: String, events: List<EventPropagatableToXml>): BpmnFileObject {
