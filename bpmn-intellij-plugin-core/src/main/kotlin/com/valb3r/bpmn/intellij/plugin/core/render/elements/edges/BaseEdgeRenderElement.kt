...
abstract class BaseEdgeRenderElement(
    ...
    override fun currentOnScreenRect(camera: Camera): Rectangle2D.Float {
        val minX = edge.waypoint.minByOrNull { it: IdentifiableWaypoint -> it.x }?.x ?: 0.0f
        val minY = edge.waypoint.minByOrNull { it: IdentifiableWaypoint -> it.y }?.y ?: 0.0f
        val maxX = edge.waypoint.maxByOrNull { it: IdentifiableWaypoint -> it.x }?.x ?: 0.0f
        val maxY = edge.waypoint.maxByOrNull { it: IdentifiableWaypoint -> it.y }?.y ?: 0.0f
    ...
    private fun drawNameIfAvailable(waypoints: List<Point2D.Float>, color: Color) {
        ...
        val longestSegment = waypoints
            ...
            .maxByOrNull { it: Pair<Point2D.Float, Point2D.Float> -> it.first.distance(it.second) } ?: return
