...
internal class UiMultiSelectTest: BaseUiTest() {
    ...
    fun `When selecting elements from multiple levels only elements from one level are chosen`() {
        ...
        canvas.dragOrSelectWithLeftButton(selectionStart, Point2D.Float(endElemX + taskSize + 10.0f, endElemY + taskSize + 10.0f))
        canvas.paintComponent(graphics)
    ...
    fun `Multiple elements can be dragged out from subprocess and will get parent process as the parent`() {
        ...
        canvas.startSelectionOrDrag(selectionStart)
        canvas.paintComponent(graphics)
        canvas.dragOrSelectWithLeftButton(selectionStart, Point2D.Float(endElemX + taskSize, endElemX + taskSize))
        canvas.paintComponent(graphics)
        canvas.stopDragOrSelect()
        canvas.paintComponent(graphics)
