...
class ProcessNode : BpmnMappable<BpmnProcess>, ProcessBody() {
    ...
    private fun applyIntermediateCatchEventCustomizationByType(process: BpmnProcessBody): BpmnProcessBody {
        var result = process
        result = extractIntermediateCatchEventsBasedOnType(result, { null != it.timerEventDefinition }, cachedMapper(TimerCatchingMapper::class.java)) { updates, target ->
            target.copy(intermediateTimerCatchingEvent = updates)
        }
        result = extractIntermediateCatchEventsBasedOnType(result, { null != it.signalEventDefinition }, cachedMapper(SignalCatchingMapper::class.java)) { updates, target ->
            target.copy(intermediateSignalCatchingEvent = updates)
        }
        result = extractIntermediateCatchEventsBasedOnType(result, { null != it.messageEventDefinition }, cachedMapper(MessageCatchingMapper::class.java)) { updates, target ->
            target.copy(intermediateMessageCatchingEvent = updates)
        }
        result = extractIntermediateCatchEventsBasedOnType(result, { null != it.conditionalEventDefinition }, cachedMapper(ConditionalCatchingMapper::class.java)) { updates, target ->
            target.copy(intermediateConditionalCatchingEvent = updates)
        }
        result = extractIntermediateCatchEventsBasedOnType(result, { null != it.linkEventDefinition }, cachedMapper(LinkIntermediateCatchMapper::class.java)) { updates, target ->
            target.copy(intermediateLinkCatchingEvent = updates)
        }
        return result
    }
    ...
    private fun applyIntermediateThrowingEventCustomizationByType(process: BpmnProcessBody): BpmnProcessBody {
        ...
        result = extractIntermediateThrowingEventsBasedOnType(result, { null != it.signalEventDefinition }, cachedMapper(SignalThrowMapper::class.java)) { updates, target ->
            target.copy(intermediateSignalThrowingEvent = updates)
        }
        result = extractIntermediateThrowingEventsBasedOnType(result, { null != it.escalationEventDefinition }, cachedMapper(EscalationThrowMapper::class.java)) { updates, target ->
            target.copy(intermediateEscalationThrowingEvent = updates)
        }
        return result
    }
    ...
    private fun applyEndEventCustomizationByType(process: BpmnProcessBody): BpmnProcessBody {
        ...
        result =
            extractEndEventsBasedOnType(result, { null != it.terminateEventDefinition }, cachedMapper(EndTerminationMapper::class.java)) { updates, target -> target.copy(terminateEndEvent = updates) }
        return result
    }
    ...
    private fun applyStartEventCustomizationByType(process: BpmnProcessBody): BpmnProcessBody {
        ...
        result =
            extractStartEventsBasedOnType(result, { null != it.messageEventDefinition }, cachedMapper(StartMessageMapper::class.java)) { updates, target -> target.copy(messageStartEvent = updates) }
    ...
    private fun applyBoundaryEventCustomizationByType(process: BpmnProcessBody): BpmnProcessBody {
        ...
        result = extractBoundaryEventsBasedOnType(result, { null != it.compensateEventDefinition }, cachedMapper(BoundaryCompensationMapper::class.java)) { updates, target ->
            target.copy(boundaryCompensationEvent = updates)
        }
        result = extractBoundaryEventsBasedOnType(result, { null != it.conditionalEventDefinition }, cachedMapper(BoundaryConditionalMapper::class.java)) { updates, target ->
            target.copy(boundaryConditionalEvent = updates)
        }
        result = extractBoundaryEventsBasedOnType(
            result,
        ...
        result = extractBoundaryEventsBasedOnType(result, { null != it.escalationEventDefinition }, cachedMapper(BoundaryEscalationMapper::class.java)) { updates, target ->
            target.copy(boundaryEscalationEvent = updates)
        }
        result = extractBoundaryEventsBasedOnType(
            result,
    ...
    @Mapper(
        ...
        SendTask.SendTaskMapping::class,ReceiveTask.ReceiveTaskMapping::class,
